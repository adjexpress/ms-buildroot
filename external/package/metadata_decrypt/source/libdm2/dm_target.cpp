...
void DmTargetVerity::SetVerityMode(const std::string& mode) {
    if (mode != "panic_on_corruption" &&
        ...
        std::cout<< "ERROR"  << "Unknown verity mode: " << mode;
        valid_ = false;
        return;
    }
...
std::string DmTargetSnapshot::GetParameterString() const {
    ...
    switch (mode_) {
        ...
        case SnapshotStorageMode::Transient:
            ...
            break;
        default:
            std::cout<< "ERROR"  << "DmTargetSnapshot unknown mode";
            break;
    }
...
bool DmTargetSnapshot::ParseStatusText(const std::string& text, Status* status) {
    ...
    if (sections.size() == 0) {
        std::cout<< "ERROR"  << "could not parse empty status";
        return false;
    }
    ...
    std::cout<< "ERROR"  << "could not parse snapshot status: wrong format";
    return false;
}
...
bool DmTargetSnapshot::GetDevicesFromParams(const std::string& params, std::string* base_device,
    ...
    if (pieces.size() < 2) {
        std::cout<< "ERROR"  << "Parameter string is invalid: " << params;
        return false;
    }
