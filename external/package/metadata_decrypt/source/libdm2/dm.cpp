...
DeviceMapper::DeviceMapper() : fd_(-1) {
    ...
    if (fd_ < 0) {
        std::cout<< "ERROR " << "Failed to open device-mapper";
    }
}
...
bool DeviceMapper::CreateDevice(const std::string& name, const std::string& uuid) {
    if (name.empty()) {
        std::cout<< "ERROR " << "Unnamed device mapper device creation is not supported";
        return false;
    }
    if (name.size() >= DM_NAME_LEN) {
        std::cout<< "ERROR " << "[" << name << "] is too long to be device mapper name";
        return false;
    }
    ...
    if (ioctl(fd_, DM_DEV_CREATE, &io)) {
        std::cout<< "ERROR " << "DM_DEV_CREATE failed for [" << name << "]";
        return false;
    }
...
bool DeviceMapper::DeleteDevice(const std::string& name,
    ...
    if (!GetDeviceUniquePath(name, &unique_path)) {
        std::cout<< "ERROR " << "Failed to get unique path for device " << name;
    }
    struct dm_ioctl io;
    InitIo(&io, name);

    if (ioctl(fd_, DM_DEV_REMOVE, &io)) {
        std::cout<< "ERROR " << "DM_DEV_REMOVE failed for [" << name << "]";
        return false;
    }
    ...
    if ((io.flags & DM_UEVENT_GENERATED_FLAG) == 0) {
        std::cout<< "ERROR " << "Didn't generate uevent for [" << name << "] removal";
        return false;
    }
    ...
    if (!WaitForFileDeleted(unique_path, timeout_ms)) {
        std::cout<< "ERROR " << "Failed waiting for " << unique_path << " to be deleted";
        return false;
    }
    return true;
}
...
bool DeviceMapper::DeleteDeviceDeferred(const std::string& name) {
    ...
    if (ioctl(fd_, DM_DEV_REMOVE, &io)) {
        std::cout<< "ERROR " << "DM_DEV_REMOVE with DM_DEFERRED_REMOVE failed for [" << name << "]";
        return false;
    }
    return true;
}
...
static std::string GenerateUuid() {
    ...
    char uuid_chars[37] = {"e602933a-fa2a-41bb-974c-7f61bfcae8cf"};
...
bool DeviceMapper::WaitForDevice(const std::string& name,
    ...
    if (!GetDeviceUniquePath(name, &unique_path) || !GetDmDevicePathByName(name, path)) {
        std::cout<< "DEBUG " << " !GetDeviceUniquePath(name, &unique_path) || !GetDmDevicePathByName(name, path) "<<std::endl;
        DeleteDevice(name);
        return false;
    }
    ...
    //         std::cout<< "ERROR " << "Detected ueventd incompatibility, reverting to legacy libdm behavior.";
    //         unique_path = *path;
    //     }
    // }

    if (!WaitForFile(*path, timeout_ms)) {
        std::cout<< "ERROR " << "Failed waiting for device path: " << unique_path;
        DeleteDevice(name);
        return false;
    }
    return true;
...
bool DeviceMapper::CreateDevice(const std::string& name, const DmTable& table, std::string* path,
    ...
    if (!CreateEmptyDevice(name)) {
        std::cout<< "DEBUG " << "CreateEmptyDevice "<< name<<" failed" <<std::endl;
        return false;
    }

    if (!LoadTableAndActivate(name, table)) {
        std::cout<< "DEBUG " << "LoadTableAndActivate failed"<<std::endl;
        DeleteDevice(name);
        return false;
    }

    if (!WaitForDevice(name, timeout_ms, path)) {
        std::cout<< "DEBUG " << "WaitForDevice"<<std::endl;
        DeleteDevice(name);
        return false;
    }
...
bool DeviceMapper::GetDeviceUniquePath(const std::string& name, std::string* path) {
    ...
    if (ioctl(fd_, DM_DEV_STATUS, &io) < 0) {
        std::cout<< "ERROR " << "Failed to get device path: " << name;
        return false;
    }

    if (io.uuid[0] == '\0') {
        std::cout<< "ERROR " << "Device does not have a unique path: " << name;
        return false;
    }
    std::cout<< " io.uuid: " << io.uuid <<std::endl;
...
bool DeviceMapper::LoadTableAndActivate(const std::string& name, const DmTable& table) {
    ...
    if (ioctl(fd_, DM_TABLE_LOAD, io)) {
        std::cout<< "ERROR " << "DM_TABLE_LOAD failed";
        return false;
    }

    InitIo(io, name);
    if (ioctl(fd_, DM_DEV_SUSPEND, io)) {
        std::cout<< "ERROR " << "DM_TABLE_SUSPEND resume failed";
        return false;
    }
    return true;
}
...
bool DeviceMapper::GetAvailableTargets(std::vector<DmTargetTypeInfo>* targets) {
    ...
    if (ioctl(fd_, DM_LIST_VERSIONS, io)) {
        std::cout<< "ERROR " << "DM_LIST_VERSIONS failed";
        return false;
    }
    ...
    if (io->flags & DM_BUFFER_FULL_FLAG) {
        std::cout<< " INFO" << data_size << " is not enough memory to list all dm targets";
        return false;
    }
...
bool DeviceMapper::GetAvailableDevices(std::vector<DmBlockDevice>* devices) {
    ...
    if (ioctl(fd_, DM_LIST_DEVICES, io)) {
        std::cout<< "ERROR " << "DM_LIST_DEVICES failed";
        return false;
    }
    ...
    if (io->flags & DM_BUFFER_FULL_FLAG) {
        std::cout<< " INFO" << data_size << " is not enough memory to list all dm devices";
        return false;
    }
...
bool DeviceMapper::GetDmDevicePathByName(const std::string& name, std::string* path) {
    ...
    InitIo(&io, name);
    if (ioctl(fd_, DM_DEV_STATUS, &io) < 0) {
        std::cout<< "WARNING" << "DM_DEV_STATUS failed for " << name;
        return false;
    }
...
bool DeviceMapper::GetTable(const std::string& name, uint32_t flags,
    ...
    for (buffer.resize(4096);; buffer.resize(buffer.size() * 2)) {
        ...
        if (ioctl(fd_, DM_TABLE_STATUS, io) < 0) {
            std::cout<< "ERROR " << "DM_TABLE_STATUS failed for " << name;
            return false;
        }
...
std::optional<std::string> DeviceMapper::GetParentBlockDeviceByPath(const std::string& path) {
    ...
    if (dir == nullptr) {
        std::cout<< "ERROR " << "Failed to open: " << slaves_dir;
        return std::nullopt;
    }
    ...
    for (auto entry = readdir(dir.get()); entry; entry = readdir(dir.get())) {
        ...
        if (!sub_device_name.empty()) {
            std::cout<< "ERROR " << "Too many slaves in " << slaves_dir;
            return std::nullopt;
        }
        sub_device_name = entry->d_name;
    }
    if (sub_device_name.empty()) {
        std::cout<< "ERROR " << "No slaves in " << slaves_dir;
        return std::nullopt;
    }
